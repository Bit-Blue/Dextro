<?php
	...
	if($_POST['count']=='1'){
		...
		$query=mysqli_query($con,"SELECT a.* FROM adm_sch_tran as a  where Gr_num ='".$grn['sch_src_grn']."'union all Select b.* from sch_tran as b where Gr_num ='".$grn['sch_src_grn']."'");
		...
		")
			...
			$query=mysqli_query($con,"SELECT a.* FROM adm_sch_tran as a where Gr_num ='".$_POST['adm_src']."' union all Select b.* from sch_tran as b where Gr_num ='".$_POST['adm_src']."'");
	}
	if(!$id){
		?>
		invalid GR number
