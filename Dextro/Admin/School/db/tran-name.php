 <?php
	include('../../db.php');
		...
		// $query=mysqli_query($con,"SELECT * FROM adm_sch_tran where pay_mode='$mode' union all Select * from sch_tran  where pay_mode='$mode' order by date DESC");
         $query=mysqli_query($con,"
		SELECT A.Name,T.Gr_num,T.Reciept, T.Amount, T.Month, T.fee_type, T.pay_mode, T.cheque_num, T. late_fee, T.reason, T.date FROM sch_tran AS T INNER JOIN admin_sch AS A ON A.unique_id=T.unique_id WHERE T.pay_mode='$mode'  UNION ALL SELECT A.Name,T.Gr_num, T.Reciept, T.Amount, T.Month, T.fee_type, T.pay_mode, T.cheque_num, T. late_fee, T.reason, T.date FROM adm_sch_tran AS T INNER JOIN admin_sch AS A ON A.unique_id=T.unique_id WHERE T.pay_mode='$mode' Order by date DESC");
